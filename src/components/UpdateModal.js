...
export default class UpdateModal extends PureComponent {
	...
	install = async () => {
		...
		if (!LocalPackage.isMandatory) {
			...
			this.setVisible(false);
			ToastAndroid && ToastAndroid.show(i18n.t('START_UPDATE'), ToastAndroid.SHORT);
		}
	}
				...
				<Animated.View style={[styles.update, {
					...
					}
					{
						...
						<View style={styles.download}>
							...
							<Text style={styles.download_tip}>{status == 1 && i18n.t('TRYING_UPDATE') || status == 2 && `${i18n.t('INSTALLING')}...` || status == 3 && i18n.t('START_UPDATE_NEXT_TIME')}</Text>
						</View>
					}
				</Animated.View>
			</Animated.View>
